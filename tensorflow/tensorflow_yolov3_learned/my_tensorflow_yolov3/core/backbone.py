...
def darknet53(input_data,trainable):
    ...
    input_data = common.convolutiaoal(inputdata,filter_shape=(3,3,32,64),
                                      trainable=trainable,name=name,downsample=True)

    for i in range(1):
        input_data = common.residual_block(input_data,64,32,64,trainable=trainable,name = "residual%d"%(i+0))
    input_data = common.convolutiaoal(input_data,filters_shape=(3,3,64,128),
    ...
    for i in range(4):
        input_data = common.convolutiaoal(input_data,1024,512,1024,
